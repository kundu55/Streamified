...
const FolderContent = ({folderDeleted,setFolderDeleted}) => {
    ...
    useEffect(()=>{
        const effect = async ()=>{
            ...
            const getAllUrls = async () => {
                ...
                allUrls.map((oneUrl)=>{
                    if(isVaildVideo(oneUrl.attributes.href)){
    ...
    const isVaildVideo = (imageSrc)=>{
        let extension = imageSrc.lastIndexOf(".")
        return supportedVideoFormats.includes(imageSrc.slice(extension))
    }
...
{contentList.map((link)=>(
                <div key={link} className="video-grid">
                    <VideoPlayer path={link}></VideoPlayer>
                </div>
        ))}
</div>
